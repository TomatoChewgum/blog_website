$(document).ready(function () {
    ...
    var photo_list = plist;

    // JSON.parse('{{ photo_list | safe }}')
    var data = [];

   console.log(photo_list[0].fields.name)

    for (var j = 0; j < photo_list.length; j++) {
      data.push(photo_list[j].fields)
    }
...
function addPhotos(data) {
  ...
  for (i=0; i<data.length; i++) {
    ...
    .replace('{img}', data[i].photo)
    .replace('{caption}', data[i].name)
    .replace('{desc}', data[i].words);
    html.push(_html);
